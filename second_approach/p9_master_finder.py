...
Options:
  ...
  --repos REPOS       Comma-separated list of repositories to search [default: all]
...
def main():
    ...
    parser.add_argument("--repos", type=str, default="all", help="Repositories to search (comma-separated)")
    parser.add_argument("--years", type=str, default="recent", help="Years to search: 'recent' (2015+), 'all', or comma-separated list")
    parser.add_argument("--min-year", type=float, default=2015.0, help="Minimum year to search when using 'recent'")
    ...
    try:
        ...
        if args.years.lower() == "all":
            ...
            # Filter for recent years only (2015+)
...
def generate_master_report(all_results, all_classifications, output_dir):
    ...
    with open(report_file, 'w') as f:
        ...
        if total_perfect > 0 or total_good > 0:
            ...
            f.write("1. Expand search to additional repositories not yet implemented\n")
            f.write("2. Consider using a looser search radius to find more potential matches\n")
            f.write("3. Focus on Herschel and Planck data which may be more suitable for detecting cold objects\n")
